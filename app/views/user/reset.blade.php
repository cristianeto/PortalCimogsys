		...
		<div class="ed-item movil-1-3 cuadro main-center">
			...
			<div class="ed-container main-center">
				...
				<div class="ed-item main-center form-group">
					...
					{{ Form::open(array('route'=>'user/reset', 'files'=>'true')) }}
					{{ Form::hidden('token', $token) }}
					{{ Form::label('corkjreo','Correo Electrónico')}}<br>
					<i class="fa fa-envelope fa-input"></i>{{ Form::email('correo', Input::old('correo'),  array('placeholder'=>'usuario@cimogsys.com','required')) }}<br>
					{{ Form::label("password", "Ingresar Contraseña: ") }}	 <br>
					<i class="fa fa-lock"></i>{{ Form::password("password", ["placeholder" => "••••••••••","required"]) }}<br>
					{{ Form::label("password_confirmation", "Repetir Contraseña: ") }} <br>	
					<i class="fa fa-lock"></i>{{ Form::password("password_confirmation", ["placeholder" => "••••••••••","required"]) }}<br>
				</div>
