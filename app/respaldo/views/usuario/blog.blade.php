	...
	@section('header')
		<header class="ed-container full">
			<div class="ed-item cross-center main-start movil-50">
				<h1>
					<a>Red de Conocimiento - Usuario</a>
				</h1>
			</div>
		</header>
	@stop
		...
		<main class="ed-container full">
			...
			<div class="ed-item tablet-60 cuerpo padding-2">
				...
				<div class="ed-container frmMensajes">
					<div class="ed-item movil-100">
						{{ Form::open(array('url'=>'/usuarios/guardarComentario')) }}
	...
	<script language="javascript">
			...
			.done(function(files) {
				...
				$.each (files, function(i,item){
					...
					if(files[i].de_comentario == usuario){ 
						clase = "main-start";
						flecha = "fa-share";
					}
					else { 
					...
					$('.mensajes').append( '<div class="ed-container actividad padding-2"> <div class="ed-item usuario cross-center '+clase+'"> <p>'+ genero(files[i].genero_usuario)+' '+files[i].nombres_usuario+ '</p> </div> <div class="ed-item archivo cross-center "> <form id='+ files[i].id_comentario+' class='+files[i].silabo_comentario+'> <label for="silabo"><i class="fa '+flecha+' "></i> &nbsp;'+files[i].detalle_comentario + '</label> </form> </div> </div>' );
				});
			});	
	...
	$(document).on('click','.submit',function(e){
		...
		.done(function(files) {
				$.each (files, function(i,item){
					...
					$('.mensajes').prepend( '<div class="ed-container actividad padding-2"> <div class="ed-item usuario cross-center '+clase+'"> <p>'+ genero(files[i].genero_usuario)+' '+files[i].nombres_usuario+ '</p> </div> <div class="ed-item archivo cross-center "> <p id='+ files[i].id_comentario+' class='+files[i].silabo_comentario+'> <label for="silabo"><i class="fa '+flecha+' "></i> &nbsp;'+files[i].detalle_comentario + '</label> </p> </div> </div>' );
				});
				frm.reset();
		});
		function genero (est){
